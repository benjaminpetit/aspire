...
public static class YarpRouteExtensions
{
    ...
    /// <summary>
    /// Set the target of the route.
    /// </summary>
    public static YarpRoute WithReference(this YarpRoute route, YarpDestination destination)
    {
        route.Configure(r => r with { ClusterId = destination.ClusterConfig.ClusterId });
        return route;
    }
    ...
    /// Set the target of the route.
    /// </summary>
    public static YarpRoute WithReference(this YarpRoute route, EndpointReference destination)
    {
        var yarpDestination = route.ParentBuilder.AddDestination(destination);
        route.Configure(r => r with { ClusterId = yarpDestination.ClusterConfig.ClusterId });
        return route;
    }
}
